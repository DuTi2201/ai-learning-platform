import { Request, Response } from 'express';
export declare class ModuleController {
    static getModulesByCourse: (req: Request, res: Response, next: import("express").NextFunction) => void;
    static getModuleById: (req: Request, res: Response, next: import("express").NextFunction) => void;
    static createModule: (req: Request, res: Response, next: import("express").NextFunction) => void;
    static updateModule: (req: Request, res: Response, next: import("express").NextFunction) => void;
    static deleteModule: (req: Request, res: Response, next: import("express").NextFunction) => void;
    static reorderModules: (req: Request, res: Response, next: import("express").NextFunction) => void;
}
//# sourceMappingURL=module.controller.d.ts.map